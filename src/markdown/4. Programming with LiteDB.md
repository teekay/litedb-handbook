...
- Bulk delete using an expression: `col.DeleteMany($"$.PlaylistId = {playlist.Id}")` (here, the collection contains IDs of tracks used in a particular playlist, and we are removing all that are associated with a particular playlist)
...
2. Fetch all tracks matching the returned IDs:
